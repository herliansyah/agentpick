...
export async function showReadmeModal() {
  const width = Math.min((process.stdout.columns || 80) - 6, 76);
  const text = readFileSync(new URL('../README.md', import.meta.url), 'utf8').trim();
  const lines = renderMarkdown(text, width);

  const top = pc.cyan('╭─ ') + pc.bold('📖 agentpick') + pc.cyan(' ' + '─'.repeat(Math.max(0, width - 11)) + '╮');
  const bottom = pc.cyan('╰' + '─'.repeat(width + 2) + '╯');
